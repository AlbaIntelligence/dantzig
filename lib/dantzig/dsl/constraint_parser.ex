defmodule Dantzig.DSL.ConstraintParser do
  ...
  def parse_value_to_polynomial(value, bindings, problem) do
    case value do
      ...
      # Handle variable references (bare atoms)
      ...
      # Handle variable AST nodes (e.g., {:n, [], context})
      {var_name, _, _} when is_atom(var_name) ->
        case Map.get(bindings, var_name) do
          nil ->
            raise ArgumentError,
                  "Variable #{var_name} not found in bindings: #{inspect(bindings)}"
  ...
  def parse_variable_access(var_name, indices, bindings, problem) do
    ...
    if var_map do
      resolved_indices =
        Enum.map(indices, fn
          ...
          {var_atom, _, _} when is_atom(var_atom) ->
            Map.get(bindings, var_atom, var_atom)
      ...
      # Handle pattern matching (with :_ wildcards) the same way as expression parser
      cond do
        Enum.any?(resolved_indices, &(&1 == :_)) ->
          # Pattern matching - find all variables that match the pattern
          matching_vars =
  ...
  defp evaluate_condition(ast, bindings) do
    case ast do
      ...
      # Boolean ops
      {:and, _, [l, r]} ->
        evaluate_condition(l, bindings) and evaluate_condition(r, bindings)

      {:or, _, [l, r]} ->
        evaluate_condition(l, bindings) or evaluate_condition(r, bindings)

      {:not, _, [x]} ->
        not evaluate_condition(x, bindings)
