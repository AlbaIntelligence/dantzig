defmodule Dantzig.Problem.DSL.ExpressionParser do
  ...
  def parse_sum_expression(expr, bindings, problem) do
    ...
    case expr do
      ...
      # NEW: Wildcard arithmetic expression handler
      # Handles expressions like: qty(:_) * foods[:_][nutrient]
      # Must come before the catch-all error to handle arithmetic with wildcards
      expr ->
        if contains_wildcard?(expr) do
          expand_wildcard_sum(expr, bindings, problem)
        else
  ...
  ## Wildcard Expansion for Nested Map Access
  ## ===========================================
  ## These functions support concise wildcard syntax like:
  ...
  defp contains_wildcard?(expr) do
    {_, found?} =
      Macro.traverse(
        expr,
        false,
  ...
  defp expand_wildcard_sum(expr, bindings, problem) do
    ...
    Enum.reduce(domain, Polynomial.const(0), fn value, acc ->
      ...
      term_poly = parse_expression_to_polynomial(inst_expr, bindings, problem)
        ...
        fn
          {{:., _, [Access, :get]}, _, [container_ast, key_ast]} = node, acc ->
            if key_ast == :_ do
              ...
              container =
                case try_evaluate_constant(container_ast, bindings) do
                  {:ok, val} -> val
                  :error ->
                    # If try_evaluate_constant fails, fall back to direct evaluation
                    evaluate_expression_with_bindings(container_ast, bindings)
                end
