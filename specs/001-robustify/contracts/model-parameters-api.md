...
**Parameter Access**: Parameters are accessed DIRECTLY by their key name from the model parameters map. Unknown symbols in generators, expressions, and descriptions are automatically looked up in the model parameters map. This matches the DSL syntax reference (`docs/user/reference/dsl-syntax.md`).
...
**Note**: Objective function uses positional argument for direction: `objective(expression, :direction)`, matching the DSL syntax reference (`docs/user/reference/dsl-syntax.md`).
