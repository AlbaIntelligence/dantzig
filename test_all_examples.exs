...
results =
  Enum.map(examples, fn example_file ->
    ...
    if not File.exists?(example_file) do
      ...
      syntax_errors =
        [] ++
          if(String.contains?(content, "Problem.new("),
            do: ["Uses deprecated Problem.new() - should use Problem.define"],
            else: []
          ) ++
          if(String.contains?(content, "objective([],"),
            do: ["Invalid objective syntax: objective([], ...) - empty list not allowed"],
            else: []
          ) ++
          if(
            String.contains?(content, "problem1.") and
              not String.contains?(content, "problem1 ="),
            do: ["References undefined variable 'problem1'"],
            else: []
          ) ++
          if Regex.match?(~r/that\(are\)/, content),
            do: ["Contains invalid syntax: that(are)"],
            else: []

      # Try to compile/run the example
      {exec_status, exec_output} =
        try do
          System.cmd("mix", ["run", example_file], stderr_to_stdout: true)
        rescue
...
syntax_issues =
  Enum.filter(results, fn result ->
    Map.has_key?(result, :syntax_errors) and result.syntax_errors != [] and
