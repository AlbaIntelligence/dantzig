...
problem =
  Problem.variables(problem, "x", for(i <- 1..4, j <- 1..4, do: {i, j}), :binary,
...
problem = Problem.constraints(problem, for(i <- 1..4, do: i), x(i, :_) == 1, "One queen per row")
...
problem =
  Problem.constraints(problem, for(j <- 1..4, do: j), x(:_, j) == 1, "One queen per column")
...
problem2 =
  Problem.variables(problem2, "x", for(i <- cities, j <- cities, do: {i, j}), :binary, description: "Edge used")
...
problem2 = Problem.constraints(problem2, for(i <- cities, do: i), x(i, :_) == 1, "Outgoing edges")
problem2 = Problem.constraints(problem2, for(i <- cities, do: i), x(:_, i) == 1, "Incoming edges")

IO.puts("Created #{map_size(problem2.constraints)} constraints for TSP")

IO.puts("\n=== Classroom Scheduling ===")

# Example 3: Classroom Scheduling
courses = [1, 2]
times = [1, 2, 3]
rooms = [1, 2]
...
problem3 =
  Problem.variables(problem3, "x", for(c <- courses, t <- times, r <- rooms, do: {c, t, r}), :binary,
...
problem3 =
  Problem.constraints(problem3, for(c <- courses, do: c), x(c, :_, :_) == 1, "Course scheduled once")

# Constraint: no room double-booking
problem3 =
  Problem.constraints(
    problem3,
    for(t <- times, r <- rooms, do: {t, r}),
    x(:_, t, r) <= 1,
