...
defmodule Dantzig.Examples.Validation do
  ...
  def validate_all_examples(opts \\ []) do
    ...
    results = Enum.map(example_files, fn file_path ->
  ...
  def validate_example_file(file_path, timeout \\ 60, check_documentation \\ true) do
    ...
    documentation_analysis = if check_documentation do
    ...
    %{
      ...
      mathematical_formulation_present: documentation_analysis.mathematical_formulation_present || false,
      dsl_syntax_explanation_present: documentation_analysis.dsl_syntax_explanation_present || false,
  ...
  defp execute_example(file_path, timeout) do
    try do
      ...
      {execution_time, {status, output}} = :timer.tc(fn ->
        # Use System.cmd to run the example with timeout
        {output, exit_status} = System.cmd("elixir", [file_path], 
  ...
  defp determine_file_status(execution_status, documentation_analysis) do
    if execution_status == :success and documentation_analysis.quality in [:comprehensive, :adequate] do
      :passed
    else
      :failed
    end
